...
// Pass a command line arg when testing locally to avoid fatals
const gitRefFromCli = process.argv[2];
const latestGitCommitHash =
  gitRefFromCli ?? childProcess.execSync('git rev-parse --short "$GITHUB_SHA"').toString().trim();

export const RAW_GITHUB_URL = `https://raw.githubusercontent.com/ava-labs/subnet-assets/${latestGitCommitHash}`;
